...
class backgroundOverlayer(object):
    ...
    def __call__(self, background_img):
        ...
        for _ in range(tags_to_overlay):
            ...
            grid_corners_coords = result["grid_corners_uv"]
            ...
            if  not cv2.bitwise_and(out_response_view, mask).any():
                ...
                grid_corners_collection.append([np.array([[x_offset, y_offset]])+grid_corners_coords ])
                bytecode_collection.append(bytecode)
                familycode_collection.append(familycode)


        return background_img, out_response, np.clip(real_out_response,0,255), corners_collection, bytecode_collection, familycode_collection, grid_corners_collection
