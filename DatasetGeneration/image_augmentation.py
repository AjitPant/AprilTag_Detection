...
class Rotation3DAugmentation(object):
    ...
    def __call__(self, src_image, src_corners):
        ...
        src_pts_img = np.ones((src_corners.shape[0], 4), dtype=np.float32)
        src_pts_img[0, :] = src_corners[:, 0]
        src_pts_img[1, :] = src_corners[:, 1]
        ...
        dst_corners = dst_pts_img[:2].T
        dst_H_src = cv2.getPerspectiveTransform(src_corners * width, dst_corners * width)
        dst_image = cv2.warpPerspective(src_image, dst_H_src, (width, height))

        return dst_image, dst_H_src, dst_corners
